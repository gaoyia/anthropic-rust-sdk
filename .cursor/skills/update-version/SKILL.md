...
版本策略：主次版本（`major.minor`）与上游 `anthropic-sdk-typescript` 对齐；patch 位由 Rust 侧自主管理，用于上游同一主次版本内的缺陷修复或兼容网关健壮性增强。更新版本时必须同步下列全部位置，缺一不可，否则会出现版本号不一致。
...
切换后读取 `anthropic-sdk-typescript/src/version.ts` 的 `VERSION` 确认上游版本；crate 主次版本与之对齐，patch 位按需（同步上游新版本时通常归零，仅 Rust 侧修复时递增）。子模块指针改动单独提交（`build(submodule): 同步子模块至 sdk-v<version>`）。
